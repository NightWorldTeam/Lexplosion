...
namespace Lexplosion.Logic.Network
{
	public class NightWorldApi
	{
		...
		public LauncherInitInfo LoadLauncherInitInfo(int timeout = 10000)
		{
			var result = _toServer.HttpGet(LaunсherSettings.URL.LauncherParts + "launcherInitInfo.html", timeout: timeout);
			if (result == null) return null;

			try
			{
				return JsonConvert.DeserializeObject<LauncherInitInfo>(result);
			}
			catch (Exception ex)
			{
				Runtime.DebugWrite($"Load inint info error {ex}");
				return null;
			}
		}
