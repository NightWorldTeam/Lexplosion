...
namespace Lexplosion.UI.WPF.Mvvm.Models.Mvvm.InstanceModel
{
	public class InstanceModelBase : ViewModelBase, IEquatable<InstanceClient>
	{
		...
		#region Downloading Fields


		private readonly System.Diagnostics.Stopwatch _progressStopwatch = System.Diagnostics.Stopwatch.StartNew();
		private int _lastPercent = -1;
		private int _lastFilesCount = -1;
		private StateType _lastState;


		#endregion Downloading Fields
		...
		{
			// Дросселирование (Throttling): отсекаем лишние вызовы в фоновом потоке.
			// Обновляем UI только если:
			// - Изменился процент скачивания (например, с 44% на 45%)
			// - Изменилось количество скачанных файлов
			// - Изменился общий статус (State)
			// - Либо прошло больше 50 миллисекунд с последнего обновления
			bool shouldUpdate = progressHandlerArguments.Procents != _lastPercent ||
								progressHandlerArguments.FilesCount != _lastFilesCount ||
								State != _lastState ||
								_progressStopwatch.ElapsedMilliseconds > 50;
			...
			// Запоминаем последнее состояние для следующей проверки
			_lastPercent = progressHandlerArguments.Procents;
			_lastFilesCount = progressHandlerArguments.FilesCount;
			_lastState = State;
			...
			App.Current.Dispatcher.BeginInvoke(new Action(() =>
			{
				...
				// Не обновляем информацию о скачивании, если количество скачиваемых файлов на стадии не получено (= 0).
				...
				// Вызов событий, на которые может быть подписан UI
